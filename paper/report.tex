...
\begin{algorithm}[H]
\caption{WedgeADJ(\textit{G}, \textit{R})}
\hspace*{\algorithmicindent} \textbf{Input:} \textit{G}: graph, \textit{R}: ranking of nodes \\
\hspace*{\algorithmicindent} \textbf{Output:} \textit{G'}: wedge adjacency list
\begin{algorithmic}[1]
    \State initialize $W$, $G'$
    \ForEach {$u1 \in U$}
        \ForEach {$v \in N(u1)$}
            \If{$R(v) > R(u1)$}
                \ForEach {$u2 \in N(v)$}
                    \If{$R(u2) > R(u1)$}
                        \ForEach {$wedge w \in W(u1)$}
                            \State $a1, a2, b = w$
                            \If{$a2 != u2 and b != v$}
                                \State add (u1, u2, v) to w in G'
                                \State add w to (u1, u2, v) in G'
                            \EndIf
                        \EndFor
                        \ForEach {$wedge w \in W(u2)$}
                            \State $a1, a2, b = w$
                            \If{$a2 != u1 and b != v$}
                                \State add (u1, u2, v) to w in G'
                                \State add w to (u1, u2, v) in G'
                            \EndIf
                        \EndFor
                        \State add (u1, u2, v) to W[u1]
                        \State add (u1, u2, v) to W[u2]
                    \Else
                        \State break
                    \EndIf
                \EndFor
